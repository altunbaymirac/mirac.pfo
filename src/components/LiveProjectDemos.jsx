...
const DEMOS = [
  {
    ...
    url: '/demos/flare', // Will be served separately
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {DEMOS.map((demo) => (
          <motion.div
            ...
            className={`bg-terminal-darker border-2 border-${demo.color} p-6 cursor-pointer group`}
            onClick={() => setActiveDemo(demo)}
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className={`text-xl font-bold text-${demo.color} mb-2`}>
                  {demo.title}
                </h3>
        ...
        {activeDemo && (
          ...
          >
            <motion.div
              ...
              className={`w-full max-w-7xl h-[90vh] bg-terminal-bg border-4 border-${activeDemo.color} relative`}
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="bg-terminal-darker p-4 border-b-2 border-terminal-text flex items-center justify-between">
                <div>
                  <h3 className={`text-xl font-bold text-${activeDemo.color} neon-glow`}>
                    {activeDemo.title}
                  </h3>
              ...
              {/* Demo Content */}
              <div className="h-[calc(100%-80px)] bg-terminal-darker">
                <iframe
                  src={activeDemo.url}
